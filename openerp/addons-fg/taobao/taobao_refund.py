...
@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundSuccess')
def TaobaoRefundSuccess(dbname, uid, app_key, rsp):
    #退款成功
    #TODO receive return goods or pay customer
    return
@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundClosed')
...
@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundSellerAgreeAgreement')
def TaobaoRefundSellerAgreeAgreement(dbname, uid, app_key, rsp):
    #卖家同意退#款协议
    pass

@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundSellerRefuseAgreement')
def TaobaoRefundSellerRefuseAgreement(dbname, uid, app_key, rsp):
    #卖家拒绝退款协议
    pass

@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundBuyerModifyAgreement')
def TaobaoRefundBuyerModifyAgreement(dbname, uid, app_key, rsp):
    #买家修改退款协议
    pass

@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundBuyerReturnGoods')
...
@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundCreateMessage')
def TaobaoRefundCreateMessage(dbname, uid, app_key, rsp):
    #发表退款留言
    pass

@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundBlockMessage')
def TaobaoRefundBlockMessage(dbname, uid, app_key, rsp):
    #屏蔽退款留言
    pass

@mq_client
@msg_route(code = 202, notify = 'notify_refund', status = 'RefundTimeoutRemind')
