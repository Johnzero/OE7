...
class _O(dict):
    ...
    def __getattr__(self, name):
        try:
            return self.get(name.lower(),False)
...
class TOP(object):
    def __init__(self, app_key = None, app_secret = None, session = None):
        ...
        #淘宝沙箱环境
        #self.top_url = 'http://gw.api.tbsandbox.com/router/rest'
        #self.stream_url = 'http://stream.api.tbsandbox.com/stream'
    ...
    def _get_top_resp(self, url, params):
        try:
            ...
            urlopen = urllib2.urlopen(url, form_data)
            rsp = urlopen.read()
    ...
    def execute(self, method_name, **kwargs):
        ...
        if curl_rsp.endswith('</html>') or curl_rsp.endswith('</html>\n'):
            raise TOPException(1000, "您所请求的服务暂时不可用。请稍后再试。")

        rsp = json.loads(curl_rsp, strict=False, object_hook =lambda x: _O(x))
        #rsp = json.loads(curl_rsp)
    ...
    def stream(self, dbname, uid, stream_id, user_id, user_nick):
        ...
        try:
            ...
            crl.setopt(pycurl.CONNECTTIMEOUT, 40)
            #设置cURL允许执行的最长秒数(12小时)
            crl.setopt(pycurl.TIMEOUT, 43200)
            ...
            _logger.warning('Taobao Stream Error:%s' % (e,))
        finally:
            crl.close()
