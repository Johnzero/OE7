...
class taobao_rate(osv.osv, TaobaoMixin):
    ...
    _columns = {
            ...
            'role': fields.selection([('seller', u'卖家'),('buyer', u'买家'),],u'评价者角色',),
            'nick': fields.char(u'评价者昵称', size=256),
            'result': fields.selection([('good', u'好评'),('neutral', u'中评'),('bad', u'差评'),],u'评价结果',),
    ...
    def rate_ticket_new(self, cr, uid, shop, top, rate, remind_user = True):
        ...
        if not partner:
            ...
            try:
                partner = res_partner_obj._save(cr, uid, args=[('taobao_nick','=', rate.nick), ('ref','=', '%s' % rate.nick)], **{'name':rate.nick, 'ref':'%s' % rate.nick, 'category_id': [shop.taobao_user_category_id.id], 'customer': True, 'taobao_user_id':tbbuyer.get('user_id', None), 'taobao_nick':rate.nick})
            except:
    ...
    def _top_traderates_get(self, top, start_date = None, end_date = None):
        ...
        while(total_results > page_no*page_size):
            ...
            if rsp and rsp.get('trade_rates', False): 
                rates = rates + rsp.trade_rates.trade_rate
