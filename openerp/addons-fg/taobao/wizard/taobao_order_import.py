...
class taobao_order_import(osv.osv_memory):
    ...
    _columns = {
            'taobao_shop_id': fields.many2one('taobao.shop', 'Taobao Shop', required=True),
            'order_begin': fields.datetime(u'起始时间', required=True),
            'order_end': fields.datetime(u'结束时间', required=True),
    ...
    def onchange_order_time(self, cr, uid, ids, order_time):
        v = {}
        if order_time:
            now = datetime.datetime(*tuple(time.gmtime())[:6]) #+ datetime.timedelta(hours=8)
            end_created = now.strftime('%Y-%m-%d %H:%M:%S')
            start_created = (now - datetime.timedelta(days=order_time)).strftime('%Y-%m-%d %H:%M:%S')
            v['order_begin'] = start_created
            v['order_end'] = end_created

        return {'value': v}

    def _internal_search(self, cr, uid, ids, context=None):
        if ids and ids[0]:
            order_import = self.browse(cr, uid, ids[0], context=context)#浏览对象ids
            ...
            if order_import.order_time == 90:
                ...
                t = datetime.datetime(*tuple(time.strptime(order_import.order_begin,'%Y-%m-%d %H:%M:%S'))[:6]) + datetime.timedelta(hours=8)
                start_created = t.strftime('%Y-%m-%d %H:%M:%S')
                t = datetime.datetime(*tuple(time.strptime(order_import.order_end,'%Y-%m-%d %H:%M:%S'))[:6]) + datetime.timedelta(hours=8)
                end_created = t.strftime('%Y-%m-%d %H:%M:%S')
    ...
    def search_order(self, cr, uid, ids, context=None):
        taobao_orders = self._internal_search(cr, uid, ids, context)

        for i in range(len(taobao_orders)):
            taobao_orders[i]['taobao_shop_id'] = context['taobao_shop_id']
    ...
    # 
    # 漏单检查
    def search_lost(self, cr, uid, ids, context=None):
        taobao_orders = self._internal_search(cr, uid, ids, context)
        lost_orders = []
        so_obj = self.pool.get('sale.order')

        for i in range(len(taobao_orders)):
            tid = taobao_orders[i]['tid']
            exist = so_obj.search(cr,uid,[('taobao_trade_id', '=','%s' % tid)])
            if not exist:
                taobao_orders[i]['taobao_shop_id'] = context['taobao_shop_id']
                lost_orders.append(taobao_orders[i].copy())
                
        context['order_import_lines'] = lost_orders
    ...
    def import_order(self, cr, uid, ids, context=None):
        if ids and ids[0]:
            ...
            for line in order_import.order_import_lines:
                ...
                #_logger.debug('SUN 001xy job=%s' % job)
                #先压入队列，等待定时器
    ...
    def direct_import(self, cr, uid, ids, context=None):
        taobao_orders = self._internal_search(cr, uid, ids, context)
        shop = self.pool.get('taobao.shop').browse(cr, uid, context['taobao_shop_id'], context=context)
        for line in taobao_orders:
            job = {"taobao_app_key": shop.taobao_app_key, "packet": {"msg": {"import_taobao_order":{"tid": line.tid, "status": line.status if line.status else None }}, "code": 9999}}
            #先压入队列，等待定时器
            TaobaoMsgRouter(cr.dbname, uid, shop.taobao_app_key, job)
